...
conexao = funcao_cadastro.conectarBancoDados(password)
...
cursor.execute("CREATE TABLE IF NOT EXISTS usuario ( id int not null, login VARCHAR(8), senha VARCHAR(4), PRIMARY KEY(id)  )")
...
try:
    while True:
        ...
        if opcao == 1:
            ...
            funcao_cadastro.removerUsuario(cursor)
            conexao.commit()
        elif opcao == 5:
